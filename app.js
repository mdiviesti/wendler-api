...
var liftweight = function (weight, val) {
    return {"orig": (weight *.9) * val, "rounded": Math.round((weight *.9) * val / 5) * 5};
};
...
var wendler = function (weight) {
    return {
        "1": {
            ...
            "set1": liftweight(weight, .65),
            "set2": liftweight(weight, .75),
            "set3": liftweight(weight, .85)
        },
        "2": {
            "week": 2,
            "title": "3-3-3+",
            "description": "2 sets of 3 progressive reps then a 3rd set of 3 to failure",
            "set1": liftweight(weight, .70),
            "set2": liftweight(weight, .80),
            "set3": liftweight(weight, .90)
        },
        "3": {
            "week": 3,
            "title": "5-3-1+",
            "description": "1 set of 5 reps, 1 set of 3 reps, then at least 1 to failure",
            "set1": liftweight(weight, .75),
            "set2": liftweight(weight, .85),
            "set3": liftweight(weight, .95)
        },
        "4": {
            "week": 4,
            "title": "Deload",
            "description": "3 sets of 5 reps",
            "set1": liftweight(weight, .40),
            "set2": liftweight(weight, .50),
            "set3": liftweight(weight, .60)
        }
    };
};
